//create a text file
//add some text to the file
#include<iostream>
#include<fstream>//to work with text files
using namespace std;
int main()
{
	//create a variable for the output stream
	ofstream output;
	
	//using this variable 'output', create a text file
	output.open("scores.txt");

	//write some data to the text file 'scores'
	output << "Jane" << " " << "T" << " " << "Doe" << " " << 78 << endl;
	output << "Kate" << " " << "A" << " " << "Adam" << " " << 65 << endl;
	output << "Anu" << " " << "B" << " " << "XYZ" << " " << 32 << endl;
	output << "Jenny" << " " << "J" << " " << "ABC" << " " << 54 << endl;

	//close the file --after writing data
	output.close();

	//confirmation message after writing data to text file
	cout << "Data written to text file"<<endl;

	//read from the text file --and display the data in the console 

	//create a variable for the input stream
	ifstream input;

	//open the file for reading --read mode 
	input.open("scores.txt");

	//declare variables for the content from the text file
	string firstname, lastname;
	char initial;
	int score;
	
	//declare a variable to count the number of records
	int count = 0;
	
	//declare a variable to total the scores
	int total = 0;
	
	//use a while loop to READ dat until last record is reached
	while (input >> firstname >> initial >> lastname >> score) 
	{
		//display this in console
		cout << "FirstName: "<<firstname << 
			"Initial: "<<initial <<
				"LastName" << lastname << 
					"Score: "<< score << endl;
		//increment the value of count
		//count = count + 1;
		//OR
		count++;

		//accumulative addition
		//total = total + score;
		//OR
		total += score;
	}
	//display the number of records 
	cout << "Total number of records: " << count << endl;

	//display the total score
	cout << "Total Score: " << total << endl;
	return 0;
}